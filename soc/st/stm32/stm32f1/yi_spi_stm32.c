...
static uint32_t yi_spi_stm32_prescaler(uint32_t source, uint32_t maximum)
{
    ...
    {
        if((source / divisors[index]) <= maximum)
        {
            return values[index];
        }
    }
...
int yi_spi_stm32_init(const void *config)
{
    const yi_spi_stm32_config_t *cfg =
        (const yi_spi_stm32_config_t *)config;
    yi_spi_stm32_data_t *data;
    uint32_t prescaler;

    if((cfg == NULL) || (cfg->self == NULL) ||
       (cfg->instance == NULL) || (cfg->self->data == NULL) ||
       !yi_device_is_ready(cfg->sck_pin) ||
       !yi_device_is_ready(cfg->miso_pin) ||
       !yi_device_is_ready(cfg->mosi_pin) ||
       (cfg->clock.enable_mask == 0U) ||
       (cfg->max_frequency == 0U) ||
    ...
    prescaler = yi_spi_stm32_prescaler(
        yi_stm32_periph_clock_rate(&cfg->clock), cfg->max_frequency);
    ...
    if((yi_pinmux_apply(cfg->sck_pin) != 0) ||
       (yi_pinmux_apply(cfg->miso_pin) != 0) ||
       (yi_pinmux_apply(cfg->mosi_pin) != 0))
    {
        (void)yi_stm32_periph_clock_disable(&cfg->clock);
        return -1;
    }

    data = (yi_spi_stm32_data_t *)cfg->self->data;
    ...
    data->hspi.Init.CLKPolarity = SPI_POLARITY_LOW;
    data->hspi.Init.CLKPhase = SPI_PHASE_1EDGE;
    ...
    data->frequency = cfg->max_frequency;
    data->mode = 0U;
                                   ...
                                   uint32_t timeout_ms)
{
    ...
    if(!yi_device_is_ready(dev) || (dev->config == NULL) ||
       (dev->data == NULL) || (config == NULL) ||
       (length == 0U) || (timeout_ms == 0U) ||
       ((tx == NULL) && (rx == NULL)))
    {
        return -1;
    }

    cfg = (const yi_spi_stm32_config_t *)dev->config;
    data = (yi_spi_stm32_data_t *)dev->data;
    if((config->frequency > cfg->max_frequency) || (config->mode > 3U))
    {
        return -1;
    }
    ...
    if((tx != NULL) && (rx != NULL))
    {
        status = HAL_SPI_TransmitReceive(&data->hspi, (uint8_t *)tx,
                                         rx, length, timeout_ms);
    }
    else if(tx != NULL)
    {
        status = HAL_SPI_Transmit(&data->hspi, (uint8_t *)tx,
                                  length, timeout_ms);
    }
    else
    {
...
void yi_spi_stm32_irq_handler(yi_device_t *dev)
{
    ...
    {
        yi_spi_stm32_data_t *data = (yi_spi_stm32_data_t *)dev->data;
        HAL_SPI_IRQHandler(&data->hspi);
    }
}

const yi_spi_api_t yi_spi_stm32_api =
{
    .transceive = yi_spi_stm32_transceive
};
