/**
 * @file yi_pinmux_stm32.c
 * @brief Configure STM32F407 and STM32H743 GPIO alternate functions.
 * @author Don
 * @date 2026-07-31
 * @version 1.0.0
 */
...
#else
#error "yi_pinmux_stm32.c requires a supported STM32 device define"
#endif

/** @brief Convert a YiCore pin mode to the STM32 HAL representation. */
...
/** @brief Convert a YiCore pull selection to the STM32 HAL representation. */
...
/** @brief Convert a YiCore speed selection to the STM32 HAL representation. */
...
/** @brief Apply one validated pin configuration to STM32 GPIO registers. */
...
/** @brief Enable the GPIO clock and initialize one pinmux device. */
int yi_pinmux_init(const void *config)
{
    const yi_pinmux_config_t *cfg = config;
...
/** @brief Reapply a registered pinmux device configuration. */
int yi_pinmux_apply(yi_device_t *dev)
{
    return (dev != NULL)
        ? yi_pinmux_apply_config((const yi_pinmux_config_t *)dev->config) : -1;
}

/** @brief Return one pin to its reset state and release its GPIO clock. */
