/**
 * @file yi_dap.h
 * @brief Expose a stable YiCore lifecycle facade for CMSIS-DAP engines.
 * @author Don
 * @date 2026-08-01
 * @version 1.0.0
 */

#ifndef YI_DAP_H
#define YI_DAP_H

#include <stdbool.h>
#include <stdint.h>

/** Backend operations implemented by the selected CMSIS-DAP engine adapter. */
typedef struct {
    int (*init)(void *context); /**< Initialize backend protocol and transport state. */
    void (*process)(void *context); /**< Process one non-blocking backend iteration. */
} yi_dap_backend_api_t;

/** YiDAP backend selection and opaque product context. */
typedef struct {
    const yi_dap_backend_api_t *backend_api; /**< Selected backend operations. */
    void *backend_context; /**< Product-owned context passed to backend operations. */
} yi_dap_config_t;

/** YiDAP lifecycle states visible to product diagnostics. */
typedef enum {
    YI_DAP_STATE_UNINITIALIZED = 0, /**< No initialization attempt has started. */
    YI_DAP_STATE_INITIALIZING, /**< The selected backend is being initialized. */
    YI_DAP_STATE_READY, /**< Initialization completed and processing is enabled. */
    YI_DAP_STATE_ERROR, /**< The most recent initialization attempt failed. */
} yi_dap_state_t;

/** Errors generated by the YiDAP lifecycle facade itself. */
typedef enum {
    YI_DAP_ERROR_NONE = 0, /**< No lifecycle error has been recorded. */
    YI_DAP_ERROR_INVALID_CONFIG = -1, /**< Configuration or callback is missing. */
    YI_DAP_ERROR_ALREADY_INITIALIZED = -2, /**< YiDAP is initializing or ready. */
} yi_dap_error_t;

/**
 * @brief Initialize the selected CMSIS-DAP engine and USB device controller.
 * @param config Immutable product configuration retained by value.
 * @return Zero on success, -1 for invalid configuration, -2 if initialized,
 * or the backend initialization error.
 * @note Thread context only; delegates transport and protocol setup to backend.
 */
int yi_dap_init(const yi_dap_config_t *config);

/**
 * @brief Process one non-blocking iteration of pending DAP and CDC work.
 * @return None.
 * @note Call continuously from thread context after yi_dap_init().
 */
void yi_dap_process(void);

/**
 * @brief Report whether the YiDAP lifecycle has completed initialization.
 * @return True after successful initialization; otherwise false.
 * @note This is lifecycle state, not USB host configuration state.
 */
bool yi_dap_is_initialized(void);

/**
 * @brief Return the current YiDAP lifecycle state.
 * @return Current state, including initialization failure state.
 * @note The state does not describe USB enumeration or target connectivity.
 */
yi_dap_state_t yi_dap_get_state(void);

/**
 * @brief Return the most recent facade or backend initialization result.
 * @return Zero when no error is recorded, otherwise a negative error code.
 * @note A new initialization attempt replaces the previously recorded error.
 */
int yi_dap_get_last_error(void);

#endif
