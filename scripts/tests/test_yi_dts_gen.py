...
class DtsGeneratorTests(unittest.TestCase):
    ...
    def test_current_board_generation(self):
        ...
        self.assertIn(".instance = (USART_TypeDef *)0x40013800U", source)
        self.assertIn(".bus = YI_STM32_BUS_APB2", source)
        self.assertIn(".tx_pin = &uart0_tx", source)
        self.assertIn(".mode = YI_PINMUX_MODE_ALTERNATE_PUSH_PULL", source)
        self.assertIn(".id = YI_STM32_CLOCK_GPIOA", source)
        self.assertIn("void USART1_IRQHandler(void)", source)
        ...
        self.assertLess(source.index("uart0_tx_cfg"), source.index("usart1_cfg"))
