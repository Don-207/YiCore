...
static int yi_flash_range_valid(const yi_flash_config_t *cfg,
                               ...
                               void *data, uint32_t length)
{
    ...
    if((data == NULL) || !yi_flash_range_valid(cfg, offset, length))
    {
        return -1;
    }
...
static int yi_stm32_flash_erase(yi_device_t *dev, uint32_t offset, uint32_t length)
{
    const yi_flash_config_t *cfg = dev->config;
    ...
    if(!yi_flash_range_valid(cfg, offset, length) ||
